...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
	...
	print("CheckInstalledApplications v5.3") 
	print("Resource Id: ", configuration_item["configuration"])
	print("Parameters: ", valid_rule_parameters)

	blacklistedApps = valid_rule_parameters["BlacklistedApps"].split(",")

	print("BlacklistedApps: ", blacklistedApps)
	...
	host = configuration_item["configuration"]["publicIpAddress"]
	...
	for blacklisted in blacklistedApps:
		if blacklisted in installed_packages:
			compliant = False
		else:
			continue
	...
	if compliant:
		...
		with open('/tmp/UnwantedApplications.json', 'w') as file:
			json.dump(mydict, file)

		s3_client.upload_file('/tmp/UnwantedApplications.json', 'my-key-bucket-rodri', 'UnwantedApplications.json')
